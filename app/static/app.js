...
function renderNetwork(data, options) {
  ...
  const tick = time => { if (run !== g.run) return; if (!g.paused) { for (let i = 0; i < nodes.length; i++) for (let j = i + 1; j < nodes.length; j++) { const a = nodes[i], b = nodes[j], dx = a.x - b.x, dy = a.y - b.y, d = Math.max(18, Math.hypot(dx, dy)), coefficient = a.type === 'excel' && b.type === 'excel' ? 7200 : a.type === 'file' && b.type === 'file' ? 1800 : a.type === 'excel' || b.type === 'excel' ? 3000 : 1800, force = coefficient / (d * d); if (!a.fixed) { a.vx += dx / d * force; a.vy += dy / d * force; } if (!b.fixed) { b.vx -= dx / d * force; b.vy -= dy / d * force; } } links.forEach(({ edge }) => { const a = map.get(edge.source), b = map.get(edge.target), dx = b.x - a.x, dy = b.y - a.y, d = Math.max(1, Math.hypot(dx, dy)), ideal = edge.kind === 'shared_columns' ? 265 : b.type === 'file' ? 118 : 185, pull = (d - ideal) * .005; if (!a.fixed) { a.vx += dx / d * pull; a.vy += dy / d * pull; } if (!b.fixed) { b.vx -= dx / d * pull; b.vy -= dy / d * pull; } }); nodes.forEach((node, index) => { if (node.fixed) return; const hx = node.homeX ?? node.x, hy = node.homeY ?? node.y; node.vx += (hx - node.x) * .002 + Math.sin(time * .001 + index) * .01; node.vy += (hy - node.y) * .002 + Math.cos(time * .0012 + index) * .01; node.vx *= .9; node.vy *= .9; node.x += node.vx; node.y += node.vy; }); } draw(); requestAnimationFrame(tick); }; draw(); requestAnimationFrame(tick);
}
