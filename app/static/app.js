...
const state = { path: null, history: [], rows: [], selected: null, mode: 'list', graph: { zoom: 1, x: 0, y: 0, run: 0, rootPath: null } };
...
function renderNetwork(data, options) {
  ...
  const elements = new Map(); nodes.forEach(node => {
    ...
    el.onclick = event => { event.stopPropagation(); if (options.kind === 'orbit' && node.type === 'folder') { if ($('#graphFiles').checked) orbitGraph(g.rootPath, node.path); else if (node !== center) orbitGraph(node.path); } else if (node.file_id) showDetail(node.file_id); }; el.ondblclick = () => node.file_id && openFile(node.file_id); elements.set(node.id, el);
  });
  ...
  let pan = false, px = 0, py = 0; canvas.onpointerdown = event => { if (event.target === canvas || event.target === layer) { pan = true; px = event.clientX; py = event.clientY; } }; canvas.onpointermove = event => { if (!pan) return; g.x += event.clientX - px; g.y += event.clientY - py; px = event.clientX; py = event.clientY; graphTransform(); }; canvas.onpointerup = () => pan = false; canvas.onwheel = event => { event.preventDefault(); g.zoom = Math.max(.35, Math.min(3, g.zoom * (event.deltaY < 0 ? 1.12 : .89))); graphTransform(); }, { passive: false };
...
function bind() {
  ...
  $('#graphRoot').onclick = () => orbitGraph(); $('#schemaGraph').onclick = excelConstellation; $('#graphFiles').onchange = () => orbitGraph(state.graph.rootPath); $('#graphZoomIn').onclick = () => { state.graph.zoom *= 1.25; graphTransform(); }; $('#graphZoomOut').onclick = () => { state.graph.zoom *= .8; graphTransform(); }; $('#graphReset').onclick = () => { state.graph.zoom = 1; state.graph.x = state.graph.y = 0; graphTransform(); };
...
async function start() { bind(); const status = await api('/api/status'); $('#statusLine').textContent = `V${status.version} · ${status.files} ficheiros`; await loadRoots(); await home(); }
