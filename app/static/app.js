...
async function excelConstellation() {
  setNav('graph'); const columns = $('#excelColumnFilter').value.trim();
  renderNetwork(await api(`/api/excel-graph?columns=${encodeURIComponent(columns)}`), { kind: 'excel' });
}
function renderNetwork(data, options) {
  ...
  $('#excelControls').classList.toggle('hidden', options.kind !== 'excel'); $('#graphFilesLabel').classList.toggle('hidden', options.kind === 'excel');
  ...
  const hint = document.createElement('div'); hint.id = 'orbitHint'; hint.className = 'orbit-hint'; hint.textContent = options.kind === 'excel' ? 'As linhas mostram as correspondências mais fortes · clique num Excel para pré-visualizar' : $('#graphFiles').checked ? 'Clique numa pasta para revelar apenas os ficheiros dela' : 'Clique numa pasta para navegar · clique num ficheiro para pré-visualizar'; canvas.append(hint);
  if (options.kind === 'excel') {
    const overview = document.createElement('aside'); overview.className = 'excel-overview';
    const selected = data.selected_columns?.length ? data.selected_columns.join(', ') : 'vista geral';
    overview.innerHTML = `<strong>Constelação de dados</strong><span>${data.nodes.length} de ${data.total_files} Excels visíveis</span><span>${data.edges.length} relações úteis · máximo 3 por Excel</span><span class="meta">Filtro: ${esc(selected)}</span><div class="common-columns"><small>Colunas frequentes</small></div>`;
    const list = overview.querySelector('.common-columns'); (data.available_columns || []).slice(0, 10).forEach(column => { const button = document.createElement('button'); button.textContent = `${column.name} · ${column.count}`; button.title = `Ver somente os Excels com a coluna “${column.name}”`; button.onclick = () => { $('#excelColumnFilter').value = column.name; excelConstellation(); }; list.append(button); }); canvas.append(overview);
    const optionsList = $('#excelColumnOptions'); optionsList.replaceChildren(...(data.available_columns || []).map(column => { const option = document.createElement('option'); option.value = column.name; option.label = `${column.name} · ${column.count} ficheiros`; return option; }));
  }
  ...
  const nodes = data.nodes.map(node => ({ ...node, x: 0, y: 0, vx: 0, vy: 0, fixed: false, radius: node.type === 'excel' ? Math.min(78, Math.max(40, 24 + node.label.length * 2.1)) : 26 })), map = new Map(nodes.map(node => [node.id, node])), children = new Map();
  ...
  if (options.kind === 'orbit') {
    ...
    // Uma grelha dá à primeira imagem uma leitura clara; a física apenas
    // acomoda os nós e aproxima relações, sem os amontoar num ponto central.
    const columns = Math.max(1, Math.ceil(Math.sqrt(nodes.length * width / Math.max(height, 1))));
    nodes.forEach((node, index) => { const column = index % columns, row = Math.floor(index / columns), rows = Math.ceil(nodes.length / columns); node.x = node.homeX = width * (column + .5) / columns; node.y = node.homeY = height * (row + .5) / rows; });
  }
  ...
  const tick = time => { if (run !== g.run) return; if (!g.paused) { for (let i = 0; i < nodes.length; i++) for (let j = i + 1; j < nodes.length; j++) { const a = nodes[i], b = nodes[j], dx = a.x - b.x, dy = a.y - b.y, d = Math.max(18, Math.hypot(dx, dy)), minimum = (a.radius || 18) + (b.radius || 18) + 18, coefficient = a.type === 'excel' && b.type === 'excel' ? 5600 : a.type === 'file' && b.type === 'file' ? 1800 : a.type === 'excel' || b.type === 'excel' ? 3000 : 1800, force = coefficient / (d * d) + Math.max(0, minimum - d) * .12; if (!a.fixed) { a.vx += dx / d * force; a.vy += dy / d * force; } if (!b.fixed) { b.vx -= dx / d * force; b.vy -= dy / d * force; } } links.forEach(({ edge }) => { const a = map.get(edge.source), b = map.get(edge.target), dx = b.x - a.x, dy = b.y - a.y, d = Math.max(1, Math.hypot(dx, dy)), ideal = edge.kind === 'shared_columns' ? 210 : b.type === 'file' ? 118 : 185, pull = (d - ideal) * .0035; if (!a.fixed) { a.vx += dx / d * pull; a.vy += dy / d * pull; } if (!b.fixed) { b.vx -= dx / d * pull; b.vy -= dy / d * pull; } }); nodes.forEach((node, index) => { if (node.fixed) return; const hx = node.homeX ?? node.x, hy = node.homeY ?? node.y; node.vx += (hx - node.x) * .003 + Math.sin(time * .001 + index) * .006; node.vy += (hy - node.y) * .003 + Math.cos(time * .0012 + index) * .006; node.vx *= .86; node.vy *= .86; node.x += node.vx; node.y += node.vy; }); } draw(); requestAnimationFrame(tick); }; draw(); requestAnimationFrame(tick);
}
